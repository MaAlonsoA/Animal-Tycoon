...
public class World {
    ...
    public void render(Graphics g) {
        ...
        int xEnd = (int) Math.min(width, (game.getGameCamera().getxOffset() + game.getScrWidth()) / Tile.TILE_WIDTH + 1);
        int yStart = (int) Math.max(0, game.getGameCamera().getyOffset() / Tile.TILE_HEIGHT);
        int yEnd = (int) Math.min(height, (game.getGameCamera().getyOffset() + game.getScrHeight()) / Tile.TILE_HEIGHT + 1);
